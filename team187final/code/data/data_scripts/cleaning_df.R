...
### Set working directory to where the repo is ##### 

setwd("~/Documents/GT Masters/CSE 6242/CSE6242-Project/team187FinalReport/")
df = read.csv("data/shot_logs.csv")


### Run the fetch_shots_by_player.R
...
for(i in 1:length(file.names)){
  file <- read.table(paste0("data/shots/", file.names[i]),header=TRUE, sep=",", stringsAsFactors=FALSE)
...
shots_log_location = read.csv("shots_log_location.csv")

height_weight = read.csv("Height_Weight.csv")

shots_log_location = shots_log_location %>% left_join(., height_weight, 
                                 by = c("player_id" = "player_id")) %>% 
  rename(attacker_height = HEIGHT, 
         attacker_weight = WEIGHT) %>% 
  left_join(., 
            height_weight, 
            by = c("CLOSEST_DEFENDER_PLAYER_ID" = "player_id")
            ) %>% 
  rename(defender_height = HEIGHT, 
         defender_weight = WEIGHT)
  
  
shots_log_location %>% data.table::fwrite(., "shots_log_location.csv")
